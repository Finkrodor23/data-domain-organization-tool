...
function createDefaultUIData(){
  return {
    version:'u0000',
    theme:{
      background:{from:'#0a0e12',to:'#0c1116',accent:'#00bc66'},
      colors:{card:'#131923',line:'#202a36',text:'#f1f5f9',muted:'#9aa7b3'},
      heatmap:[{at:0,color:'#ff4d4f'},{at:50,color:'#ffd666'},{at:100,color:'#52c41a'}],
      layout:{domainColumns:5}
    },
    vocab:{storages:[],ingestions:[],frontends:[]},
    _doc:null
  };
}
function ensureUIData(){
  if(!UI.data){
    UI.data=createDefaultUIData();
    applyTheme(UI.data);
  }
  return UI.data;
}
...
async function loadDefaultUIConfig(){try{const r=await fetch('./assets/ui-config.xml',{cache:'no-store'});if(r.ok){const t=await r.text();UI.xml=new DOMParser().parseFromString(t,'application/xml');UI.data=parseConfigXML(t);applyTheme(UI.data);if(window.setStatus)setStatus(`Loaded ui-config ${UI.data.version}`)}}catch(e){if(window.setStatus)setStatus('No ui-config.xml found')}finally{ensureUIData();}}
async function openUIConfig(){try{const[h]=await window.showOpenFilePicker({types:[{description:'XML',accept:{'text/xml':['.xml']}}]});UI.handle=h;const t=await (await h.getFile()).text();UI.xml=new DOMParser().parseFromString(t,'application/xml');UI.data=parseConfigXML(t);applyTheme(UI.data);if(window.setStatus)setStatus(`Opened ui-config ${UI.data.version}`)}catch(e){ensureUIData();if(e.name!=='AbortError')alert('Open ui-config failed: '+e.message)}}
