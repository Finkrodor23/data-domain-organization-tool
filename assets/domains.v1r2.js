...
function openDatasetDetails(ds){
  ...
  fillSel(stSel, UI.data?.vocab?.storages||[], ds.layer.storage); fillSel(inSel, UI.data?.vocab?.ingestions||[], ds.layer.ingestion); fillSel(feSel, UI.data?.vocab?.frontends||[], ds.layer.frontend);
  // org selector
  const sel=$('#dt-org'); sel.innerHTML=''; sel.appendChild(h('option',{value:'all'},'All orgs')); for(const o of Object.values(state.data.orgs)) sel.appendChild(h('option',{value:o.id},o.name)); sel.value= state.filters.org==='all' ? 'all' : state.filters.org;
  // fill fields based on org scope
  ...
  function fill(orgId){
    const s = snapshotFrom(orgId);
    desc.value=s.description||''; owner.value=s.owner||''; entries.value=s.entries||0;
    q.value=s.measures.quality||0; t.value=s.measures.timeliness||0; a.value=s.measures.accessibility||0; c.value=s.measures.completeness||0;
    fillSel(stSel, UI.data?.vocab?.storages||[], s.layer.storage); fillSel(inSel, UI.data?.vocab?.ingestions||[], s.layer.ingestion); fillSel(feSel, UI.data?.vocab?.frontends||[], s.layer.frontend);
  }
  fill(sel.value);
  sel.onchange=()=> fill(sel.value);
  // membership editor
  const membership=$('#dt-membership'); const rebuild=()=>{
    ...
    for(const oid of ids){ const present=ds.orgs?.includes(oid); const o=state.data.orgs[oid];
      const row=h('div',{style:'display:flex;align-items:center;justify-content:space-between;gap:8px;padding:4px 0;border-bottom:1px solid var(--line)'},
        h('div',{}, o.name),
        h('div',{}, present ? h('button',{class:'btn',onclick:()=>{ if(confirm('Remove from org?')){ ds.orgs=ds.orgs.filter(x=>x!==oid); delete ds.orgMeta?.[oid]; rebuild(); render(); } }}, 'Remove')
                            : h('button',{class:'btn',onclick:()=>{ ds.orgs=ds.orgs||[]; if(!ds.orgs.includes(oid)) ds.orgs.push(oid); ds.orgMeta=ds.orgMeta||{}; ds.orgMeta[oid]=ds.orgMeta[oid]||{description:ds.description,owner:ds.owner,entries:ds.entries,measures:{...ds.measures},layer:{...ds.layer}}; rebuild(); render(); }}, 'Add'))
  ...
  // Save/Delete buttons
  $('#dt-save').onclick=()=>{
    const target=sel.value; // 'all' = global, else org override
    const payload={ description:desc.value.trim(), owner:owner.value.trim(), entries:Number(entries.value||0), measures:{quality:Number(q.value||0), timeliness:Number(t.value||0), accessibility:Number(a.value||0), completeness:Number(c.value||0)}, layer:{ storage:stSel.value||'', ingestion:inSel.value||'', frontend:feSel.value||'' } };
    if(target==='all'){
      ...
      ds.orgMeta = ds.orgMeta||{}; ds.orgMeta[target] = payload; if(!ds.orgs?.includes(target)){ ds.orgs=ds.orgs||[]; ds.orgs.push(target); }
    }
...
function rebuildDropdownWithAdd(container,label,options,current,onChange){
  container.innerHTML='';
  const lab=h('label',{}, label);
  const sel=h('select',{}); for(const v of options) sel.appendChild(h('option',{value:v},v)); if(current && !options.includes(current)){ sel.appendChild(h('option',{value:current},current)); }
  sel.value=current||options[0]||''; sel.onchange=()=> onChange(sel.value);
  const add=h('button',{class:'btn',type:'button',onclick:()=>{ const v=prompt(`Add new value for ${label}:`); if(v && v.trim()){ const nv=v.trim(); sel.appendChild(h('option',{value:nv},nv)); sel.value=nv; onChange(nv); sel.focus(); } }}, '+');
...
function openAddDatasetModal(domainId){
  ...
  rebuildDropdownWithAdd($('#add-frontend'),'Frontend',UI.data?.vocab?.frontends||[], '', v=>f.dataset.frontend=v);
...
function toCSVRows(m){const rows=[['orgId','domainId','domainName','datasetId','datasetName','description','owner','entries','quality','timeliness','accessibility','completeness','storage','ingestion','frontend','useCaseGroups','useCases']];for(const d of m.domains){for(const sid of d.sets){const s=m.datasets[sid];if(!s)continue;const ucg=Array.from(new Set((s.useCases||[]).map(r=>r.split('/')[0])));const u=s.useCases||[];if(s.orgs?.length){for(const oid of s.orgs){const meta=s.orgMeta?.[oid]||{};rows.push([oid,d.id,d.name,s.id,s.name,meta.description||s.description,meta.owner||s.owner,meta.entries||s.entries,meta.measures?.quality??s.measures.quality,meta.measures?.timeliness??s.measures.timeliness,meta.measures?.accessibility??s.measures.accessibility,meta.measures?.completeness??s.measures.completeness,meta.layer?.storage||s.layer.storage,meta.layer?.ingestion||s.layer.ingestion,meta.layer?.frontend||s.layer.frontend,ucg.join('|'),u.join('|')])}}else{rows.push(['',d.id,d.name,s.id,s.name,s.description,s.owner,s.entries,s.measures.quality,s.measures.timeliness,s.measures.accessibility,s.measures.completeness,s.layer.storage,s.layer.ingestion,s.layer.frontend,ucg.join('|'),u.join('|')])}}}return rows}
...
async function boot(){await loadDefaultUIConfig(); try{const r=await fetch('./assets/sample-data.xml',{cache:'no-store'}); if(r.ok){const t=await r.text(); state.xmlDoc=parseXML(t); state.data=loadModel(state.xmlDoc); render(); }}catch(e){ setStatus('Failed to load sample-data.xml'); }
  $('#btn-open').onclick=openXML; $('#btn-save').onclick=saveXML; $('#btn-export').onclick=exportCSV; $('#fab-settings').onclick=openSettings; $('#btn-settings').onclick=openSettings; }
